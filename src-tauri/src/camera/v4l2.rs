...
use crate::camera::controls::enable_auto_image_controls;
...
impl CameraCapture {
    ...
    ///
    /// `controls` is applied once at capture start to bias the image
    /// toward a well-lit subject; see [`crate::camera::controls`] for
    ...
    pub fn stop(&mut self) {
        ...
        if let Some(handle) = self.capture_thread.take() {
            // Join failure here only happens if the capture thread
            // panicked; we still want stop() to be infallible because
            // it runs on the user-driven "stop camera" path.
            let _ = handle.join();
        }
    }
...
fn capture_loop(
    ...
    // Bias the camera toward a well-lit subject (auto-exposure on,
    // backlight compensation maxed, brightness/contrast lifted). Best
    // effort: cameras vary widely in what they actually expose so
    // failures are folded into the returned report and we move on.
    let _report = enable_auto_image_controls(&dev, controls);
